...
def verify_otp(request):
    if request.method == "POST":
        ...
        if otp_entered == otp_saved:
            ...
            return redirect('dashboard')  # Redirect to user home after successful verification
        else:
            # Invalid OTP, show error message or redirect back to OTP entry page
            return render(request, 'ecommerceapp/verify_otp.html', {'error': 'Invalid OTP. Please try again.'})
    # If request method is not POST, render the OTP verification page
    return render(request, 'ecommerceapp/verify_otp.html')
